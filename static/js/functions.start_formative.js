...
$(document).ready(function() {
  $("a.start-formative").click(function(event) {
      ...
      $.ajax({
        ...
        .done(function(data) {
          $.each(data, function(index, course) {
            ...
            html +=
              '<div class="card-item col s12 hoverable click" data-course-id=' +
              index +
              ">";
          ...
          $.each($(".step-2 .row.last-row.course-list .card-item"), function(
            index,
            val
          ) {
            $(val).click(function(event) {
              courseID = $(event.currentTarget).data("course-id");
              $(".step-2 .row.last-row.course-list .card-item").removeClass(
                "active"
              );
    ...
    if ($(event.currentTarget).data("formative-name") != undefined) {
      ...
      $(".modal#start_formative .formative-name").text(
        $(event.currentTarget).data("formative-name")
      );
  ...
  $("#manual_time").change(function(event) {
    ...
    if (formativeTime == 5 || formativeTime == 10 || formativeTime == 15) {
      $(".step-3 .row.last-row .card-item").removeClass("active");
      $(
        ".step-3 .row.last-row .card-item[data-time='" + formativeTime + "']"
      ).addClass("active");
    } else {
      $(".step-3 .row.last-row .card-item").removeClass("active");
    }
