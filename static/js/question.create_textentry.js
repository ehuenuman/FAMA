...
$(document).ready(function() {
  ...
  $(":file").change(function() {
    ...
    if (fileSize <= 1400000) {
      ...
      var input_foto = $("#input_foto");
      input_foto.replaceWith(
        input_foto.val("Tamaño máximo permitido: 1.4Mb").clone(true)
      );
  ...
  function fileOnload(e) {
    ...
    $("#imagen-desplegada").append(
      "<img style='top:-20px;position: relative;' src='" +
        result +
        "' width='350px'/>"
    );
  ...
  $("button[data-action='preview_question']").click(function() {
    ...
    $("#banner_preview h5.white-text").append("Completar texto");

    if (titulo != "")
      $("#content_preview").append(
        "<span><b>Titulo: </b>" + titulo + "</span></br>"
      );
    else
      $("#content_preview").append(
        "<span><b>Titulo: </b>* No tiene t&iacute;tulo *</span></br>"
      );

    if (texto_alternativo != "")
      $("#content_preview").append(
        "<span><b>Texto alternativo: </b>" + texto_alternativo + "</span></br>"
      );
    else
      $("#content_preview").append(
        "<span><b>Texto alternativo: </b>* No tiene texto alternativo *</span></br>"
      );

    if (nombre_foto != "") {
      $("#content_preview").append("<span><b>Imagen:</span></br>");
      $("#content_preview").append(
        "<center><img src='" + result + "' width='250px'/></center>"
      );
    }

    if (pregunta != "")
      $("#content_preview").append(
        "<span><b>Pregunta: </b>" + pregunta + "</span></br>"
      );
    else
      $("#content_preview").append(
        "<span><b>Pregunta: </b>* No tiene pregunta *</span></br>"
      );
...
function manage_question() {
  ...
  if (nombre_foto != "") {
    ...
    if (descargar) {
      var xml_file = new File([xml_question], titulo + ".xml", {
...
function crear_imsmanifestInline() {
  ...
  cH.writeAttributeString(
    "xsi:schemaLocation",
    "http://www.imsglobal.org/xsd/imscp_v1p1 imscp_v1p1.xsd http://www.imsglobal.org/xsd/imsmd_v1p2 imsmd_v1p2p4.xsd http://www.imsglobal.org/xsd/imsqti_metadata_v2p1  http://www.imsglobal.org/xsd/qti/qtiv2p1/imsqti_metadata_v2p1.xsd"
  );
